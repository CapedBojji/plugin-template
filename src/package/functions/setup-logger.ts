...
const STACK_TRACE_LEVEL_MATTER = 4;
...
class LogEventSFTOutputSink implements ILogEventSink {
	public Emit(message: LogEvent): void {
		...
		if (message.Level >= LogLevel.Fatal) error(formatted_message);
		else if (message.Level >= LogLevel.Warning) warn(formatted_message);
	...
	private getFileInformation(context: string): string {
		...
		const source =
			context === "Game"
				...
				: debug.info(STACK_TRACE_LEVEL_MATTER, "sl");
